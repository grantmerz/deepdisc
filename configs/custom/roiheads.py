...
class RedshiftPDFCasROIHeads(CascadeROIHeads):
    """
    CascadeROIHead with added redshift pdf estimation capability.  Follows the detectron2 CascadeROIHead class init, except for
    a few extra parameters.  Assumes all annotated objects have a redshift.
    ...
    zloss_factor : float
        weight applied to the redshift loss
    ...
    def _forward_redshift(self, features, instances, targets=None, image_wcs=None):
        ...
        if self.training:
            ...
            if np.all([len(inst)==0 for inst in instances]):
