...
def read_image_dc2(
    ...
    """
    Read in a formatted simulated DC2 image
    -----------
    filename: 
        filename of the target band files
        
    bands: list
        The list of the target bands
        
    
    Returns
    -------
    Orinial image without any scaling
    """
    ...
    length, width = g.shape
    image = np.empty([length, width, len(bands)], dtype=np.float64)
    
    
    catagory = filename.split(os.sep)[-5]
    tract = filename.split(os.sep)[-4]
    patch = filename.split(os.sep)[-3]
    sp = filename.split(os.sep)[-2]
    ...
    if normalize.lower() == "zscore":
        ...
        Imean = np.nanmean(I)
        Isigma = np.nanstd(I)
        image = A * (image - Imean - m) / Isigma
    
    elif normalize.lower() == "astrolupton":
        #print(image.dtype)
        image[:,:,:3] = make_lupton_rgb(image[:,:,0:1], image[:,:,1:2], image[:,:,2:3], minimum=m, stretch=stretch, Q=Q)
        image[:,:,3:] = make_lupton_rgb(image[:,:,3:4], image[:,:,4:5], image[:,:,5:6], minimum=m, stretch=stretch, Q=Q)

    return image.astype('float32')

def read_image_dc2_psf(
    filename,
    bands = ['u','g','r','i','z','y'],
    m= 0, 
    A=1e4,
    ...
    g = fits.getdata(os.path.join(filename + "_"+bands[0]+".fits"), memmap=False)
    length, width = g.shape
    image = np.empty([length, width, len(bands)], dtype=np.float64)
    ...
    if normalize.lower() == "zscore":
        ...
        #print(image.dtype)
        image[:,:,:3] = make_lupton_rgb(image[:,:,0:1], image[:,:,1:2], image[:,:,2:3], minimum=m, stretch=stretch, Q=Q)
        #image[:,:,3:6] = make_lupton_rgb(image[:,:,3:4], image[:,:,4:5], image[:,:,5:6], minimum=m, stretch=stretch, Q=Q)
