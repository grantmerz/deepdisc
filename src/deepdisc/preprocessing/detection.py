...
def run_scarlet_hsc(
    ...
    if catalog is None:
        ...
        wavecat=False
        #Sigma = np.stack((catalog["i_sdssshape_shape11"], catalog["i_sdssshape_shape12"], 
        #                  catalog["i_sdssshape_shape12"], catalog["i_sdssshape_shape22"]), axis=-1).reshape(-1, 2, 2)
        #eigenvalues = np.linalg.eigvalsh(Sigma)
        #Rs = np.sqrt(eigenvalues[:,0]**2+eigenvalues[:,1]**2)
        Rs = np.power(catalog["i_sdssshape_shape11"].values*catalog["i_sdssshape_shape22"].values-catalog["i_sdssshape_shape12"].values**2, 0.25)
        spread = Rs / sigma_obs
        spread[np.isnan(spread)] = 0.99
        print(spread)
        print("Source catalog has ", len(catalog), "objects")
